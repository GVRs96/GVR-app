            ...
            {[...payments].sort((a,b) => (b.date||"").localeCompare(a.date||"")).map(p => (
              ...
              <div key={p.id} style={{
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 6, flexShrink: 0 }}>
                  <span style={{ fontSize: 13, fontWeight: 800, color: "#79B391", whiteSpace: "nowrap" }}>+{fmtT(p.amount)} т</span>
                  <button
                    ...
                    aria-label="Изменить"
                    style={{ flexShrink:0, width:28, height:28, display:"flex", alignItems:"center", justifyContent:"center", background:"transparent", border:"1px solid var(--w10)", color:"var(--w45)", borderRadius:7, cursor:"pointer", fontFamily:"inherit", padding:0 }}
                  ><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><path d="M12 20h9"/><path d="M16.5 3.5a2.1 2.1 0 0 1 3 3L7 19l-4 1 1-4Z"/></svg></button>
                  <button
                    onClick={() => {
...
function LeadDetailModal({ lead: initLead, onClose, onSave }) {
  ...
  // Свайп вниз по шапке/ручке — закрыть шторку
  const [dragY, setDragY] = useState(0);
  const dragRef = useRef({ start: null, cur: 0 });
  const onDragStart = (e) => { dragRef.current = { start: e.touches[0].clientY, cur: 0 }; };
  const onDragMove = (e) => {
    if (dragRef.current.start == null) return;
    const dy = e.touches[0].clientY - dragRef.current.start;
    if (dy > 0) { dragRef.current.cur = dy; setDragY(dy); }
  };
  const onDragEnd = () => {
    if (dragRef.current.start == null) return;
    const dy = dragRef.current.cur;
    dragRef.current = { start: null, cur: 0 };
    if (dy > 110) save(); else setDragY(0);
  };
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        {/* Крупный крестик в углу */}
        <button onClick={save} aria-label="Закрыть" style={{
          position:"absolute", top:10, right:12, zIndex:6,
          width:36, height:36, borderRadius:"50%",
          background:"var(--w08)", border:"1px solid var(--w10)",
          color:"var(--w55)", fontSize:19, lineHeight:1, cursor:"pointer",
          display:"flex", alignItems:"center", justifyContent:"center", fontFamily:"inherit", padding:0,
        }}>✕</button>
        {/* Ручка + зона свайпа вниз для закрытия */}
        <div onTouchStart={onDragStart} onTouchMove={onDragMove} onTouchEnd={onDragEnd}
          style={{ display:"flex", justifyContent:"center", padding:"12px 0 6px", flexShrink:0, touchAction:"none", cursor:"grab" }}>
          <div style={{ width:40, height:5, borderRadius:3, background:"var(--w20)" }}/>
        </div>

        {/* Header */}
        <div onTouchStart={onDragStart} onTouchMove={onDragMove} onTouchEnd={onDragEnd} style={{
          padding:"2px 56px 14px 18px", borderBottom:"1px solid var(--w07)",
          display:"flex", alignItems:"center", gap:12, flexShrink:0, touchAction:"none",
        }}>
          <div style={{ flex:1, minWidth:0 }}>
