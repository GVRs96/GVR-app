    ...
    (async () => {
      ...
      try {
        ...
        if (files.length > 0) {
          ...
          // Drive empty — check if initialData has content and upload it
          const hasData = Array.isArray(initialData) ? initialData.length > 0 : (initialData && Object.keys(initialData).length > 0);
          if (hasData) {
            const payload = { [key]: initialData, savedAt: new Date().toISOString(), source: "localStorage_migration" };
            const res = await driveCreateFile(token, payload);
            if (res.id) { setFileId(res.id); setLastSync(new Date()); }
          }
        }
      } catch(e) {}
      setSyncing(false);
  ...
  const saveToDrive = useCallback((newOrders, newDebts) => {
    ...
    driveSaveTimer.current = setTimeout(async () => {
      ...
      try {
        ...
        const payload = { orders: newOrders, debts: newDebts, leads, savedAt: new Date().toISOString() };
