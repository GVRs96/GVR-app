...
function DispatchApp() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        const files = (list.files || []);
        if (files.length > 0) {
          // Drive has data — load it (overrides local)
          const fid = files[0].id;
          setDriveFileId(fid);
          ...
          if (cloud) {
            ...
            const localOrders = JSON.parse(localStorage.getItem("dispatch_orders") || "[]");
            // Use whichever has more orders (protects against Drive having stale data)
            if (cloudOrders.length >= localOrders.length) {
              if (cloud.orders) { setOrders(cloud.orders);  localStorage.setItem("dispatch_orders",   JSON.stringify(cloud.orders)); }
              if (cloud.debts)  { setDebts(cloud.debts);    localStorage.setItem("dispatch_debts_v2", JSON.stringify(cloud.debts)); }
              if (cloud.leads)  { localStorage.setItem("leads_data_v2", JSON.stringify(cloud.leads)); }
              // Restore settings
              if (cloud.settings) {
              ...
              // Local has more — push local to Drive to bring it up to date
              const localDebts = JSON.parse(localStorage.getItem("dispatch_debts_v2") || "{}");
              const localLeads = JSON.parse(localStorage.getItem("leads_data_v2") || "[]");
              const payload = { orders: localOrders, debts: localDebts, leads: localLeads, savedAt: new Date().toISOString(), source: "local_wins" };
              await driveUpdateFile(auth.token, fid, payload);
            }
            setDriveLastSync(new Date());
          }
        } else {
          // Drive is empty — auto-upload whatever is in localStorage
          ...
          if (hasData) {
            const payload = {
              orders: localOrders, debts: localDebts, leads: localLeads,
              savedAt: new Date().toISOString(), source: "localStorage_migration",
            };
  ...
  // Save to Drive (debounced 2s) — uses refs to always get latest fileId/token
  const saveToDrive = useCallback((newOrders, newDebts) => {
    ...
    driveSaveTimer.current = setTimeout(async () => {
      ...
      try {
        const leads   = JSON.parse(localStorage.getItem("leads_data_v2") || "[]");
        // Include settings so they sync across devices
        const settings = {
        ...
        const payload = { orders: newOrders, debts: newDebts, leads, settings, savedAt: new Date().toISOString() };
  ...
  const updateOrderWithSync = async (updatedOrder, prevOrder) => {
    ...
    await autoSyncToSheets(updatedOrder);
    ...
    const newNext  = updatedOrder.nextDate || "";
    if (newNext && newNext !== prevNext) {
      // Проверяем — нет ли уже заказа-повтора с этой датой от этого же родителя
      const alreadyExists = orders.some(
        o => o.parentId === updatedOrder.id && o.date === newNext
      );
      if (!alreadyExists) {
        const repeatOrder = {
          id:         "ORD-" + Date.now().toString(36).toUpperCase() + "R",
          parentId:   updatedOrder.id,            // ссылка на исходный заказ
          ...
          date:       newNext,
          hour:       updatedOrder.hour        || "",
          message:    updatedOrder.message     || "",
          summary:    (updatedOrder.summary    || "Заказ") + " (повтор)",
    ...
    // If reminderDate added/changed → push to reminders calendar
    const prevReminder = prevOrder?.reminderDate || "";
    const newReminder  = updatedOrder.reminderDate || "";
    if (newReminder && newReminder !== prevReminder) {
      ...
      if (token) {
        calendarCreateEvent(token, {
          ...
          date: newReminder,
          isReminder: true,
          summary: (updatedOrder.summary || "Напоминание"),
        }).catch(() => {});
      }
    }
  };
        ...
        <div style={{display:"flex",alignItems:"center",gap:8}}>
          ...
          {auth?.token && (
            <div style={{
              ...
              border: `1px solid ${driveSyncing ? "rgba(91,154,255,0.3)" : driveLastSync ? "rgba(52,199,89,0.25)" : "transparent"}`,
              ...
              {driveSyncing ? "Синхр..." : driveLastSync ? "Drive ✓" : auth.user?.email?.split("@")[0] || "●"}
            </div>
          )}
          {syncStatus && (
            <div style={{
