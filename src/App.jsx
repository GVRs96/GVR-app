          ...
          <div style={{marginTop:10, paddingTop:9, borderTop:"1px solid rgba(255,255,255,0.05)"}}>
            {order.reminderDate ? (
              <div style={{
                ...
                <button onClick={()=>onUpdate({...order,reminderDate:""})} style={{
              ...
              <div style={{display:"flex",gap:6,alignItems:"center"}}>
                <input
                  ...
                  onChange={e=>{
                    if (e.target.value) {
                      onUpdate({...order,reminderDate:e.target.value});
...
function DebtsView({ orders, debts, setDebts }) {
  ...
  return (
    <div style={{flex:1,overflowY:"auto",WebkitOverflowScrolling:"touch",padding:"18px 20px 80px",paddingBottom:"calc(120px + env(safe-area-inset-bottom))"}}>
      <div style={{ marginBottom: 16 }}>
...
function DispatchApp() {
  ...
  const autoSyncToCalendar = async (order) => {
    ...
    try {
      await createCalendarEvent(order.service, order.data || order, order.message || order.summary || "");
    } catch(e) {
      // тихо — Calendar не критичен при автосинхронизации
    }
  };

  // Обёртка для обновления заказа с автосинхронизацией
  const updateOrderWithSync = async (updatedOrder) => {
    ...
    <div style={{
      ...
      {selectedOrder && (
        <OrderDetailModal
          order={selectedOrder}
          onClose={() => setSelected(null)}
          onSave={(updated) => { saveOrder(updated); setSelected(null); }}
        />
      )}
    </div>
  );
