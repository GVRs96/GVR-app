      ...
      <div style={{ maxWidth:420, width:"100%" }}>
        ...
        <div style={{ textAlign:"center", marginBottom:36 }}>
          <div style={{
            ...
            background:"linear-gradient(135deg,#1a3a6e,#0e1f3d)",
          ...
          {!showSetup ? (
            <>
              ...
              <div style={{
                ...
                ⚠️ <strong>Google вход недоступен внутри Claude</strong><br/>
              ...
              <button onClick={()=>setShowSetup(true)} style={{
                ...
                background:"transparent", border:"1px solid #1a2a40",
            ...
            <>
              ...
              {[
                ...
                <div key={s.n} style={{display:"flex",gap:12,marginBottom:14}}>
                  <div style={{
                    ...
                    background:"#162a50",color:"#6aadff",
              ...
              <div style={{ display:"flex", gap:8 }}>
                <button onClick={()=>{ auth.saveClientId(cid); setShowSetup(false); }} style={{
                  flex:1, padding:"10px", background:"#162a50", border:"1.5px solid #2a5aad",
                  color:"#6aadff", borderRadius:10, fontSize:13, fontWeight:700,
                  cursor:"pointer", fontFamily:"inherit",
                }}>Сохранить</button>
                <button onClick={()=>setShowSetup(false)} style={{
                  padding:"10px 16px", background:"transparent", border:"1px solid #1a2a40",
    ...
    <div style={{ position:"relative" }}>
      <button onClick={()=>setShowMenu(!showMenu)} style={{
        ...
        {user?.picture
          ...
          : <div style={{width:22,height:22,borderRadius:"50%",background:"#162a50",display:"flex",alignItems:"center",justifyContent:"center",fontSize:11}}>👤</div>
        }
      ...
      {showMenu && (
        <div style={{
          ...
          background:"#0e1828", border:"1px solid #1a2a40", borderRadius:12,
...
// ─── SERVICES CONFIG ────────────────────────────────────────────────────────
const SERVICES = [
  { id: "trash",   icon: "🗑️", label: "Вывоз мусора",       color: "#FF6B35" },
  { id: "sawdust", icon: "🪵", label: "Опилки россыпью",     color: "#FFB800" },
  { id: "offcuts", icon: "🪚", label: "Обрезки доски",       color: "#30A96F" },
        ...
        <Field label="Дата">
          <input
            ...
            style={{
              ...
              border:"1.5px solid #1e2d4a", borderRadius:11,
              color: f.date ? "rgba(255,255,255,0.85)" : "rgba(255,255,255,0.4)",
...
function Field({ label, children }) {
  return (
    <div>
      <div style={{ fontSize: 11, fontWeight: 700, color: "#6b7a99", letterSpacing: "0.07em", textTransform: "uppercase", marginBottom: 6 }}>{label}</div>
      {children}
    </div>
  );
}
function Input({ placeholder, value, onChange, type = "text", step, prefix, suffix, multiline, rows = 3 }) {
  ...
  return (
    <div style={{ position: "relative", display: "flex", alignItems: "center" }}>
      {prefix && <span style={{ position:"absolute", left:10, color:"rgba(255,255,255,0.4)", fontSize:12, pointerEvents:"none", userSelect:"none" }}>{prefix}</span>}
      <input
        type={type}
        step={step}
        ...
        onBlur={e => e.target.style.borderColor = "rgba(255,255,255,0.09)"}
      />
      {suffix && <span style={{ position:"absolute", right:10, color:"rgba(255,255,255,0.4)", fontSize:12, pointerEvents:"none" }}>{suffix}</span>}
    </div>
  );
}
function SelectPick({ options, value, onChange }) {
  return (
    <select
      ...
      style={{
        width:"100%", background:"#1C1C1E",
        border:"1.5px solid #1e2d4a", borderRadius:12,
      ...
      {options.map(([label, val]) => (
        <button
          ...
          style={{
            padding: "7px 14px",
            borderRadius: 8,
            border: value === val ? "1.5px solid #2a5aad" : "1.5px solid #1e2d4a",
            background: value === val ? "rgba(0,122,255,0.12)" : "#1C1C1E",
            color: value === val ? "#007AFF" : "#5a6a88",
            fontSize: 13,
...
function OrderCard({ order, onUpdate, onDelete, onRepeat }) {
  ...
  const fieldStyle = {
    width: "100%", background: "#111113", border: "1.5px solid #1e2d4a",
  ...
  return (
    <div style={{
      background:"#141416", border:`1px solid ${editMode ? "rgba(0,122,255,0.6)" : "rgba(255,255,255,0.07)"}`,
      ...
      <div onClick={()=>{ if (!editMode) setOpen(!open); }} style={{
        ...
        <div style={{width:7,height:7,borderRadius:"50%",background:st.color,flexShrink:0,boxShadow:`0 0 6px ${st.color}88`}}/>
        <div style={{flex:1,minWidth:0}}>
          ...
          <div style={{fontSize:10,color:"rgba(255,255,255,0.38)",marginTop:2}}>
            ...
            {order.nextDate ? <span style={{color:"#FF9F0A",marginLeft:4}}>· 🔁 {new Date(order.nextDate).toLocaleDateString("ru-RU")}</span> : ""}
          </div>
        </div>
      ...
      {open && !editMode && (
        <div style={{borderTop:"1px solid #1a2540",padding:"10px 12px"}}>
          <div style={{fontSize:10,fontWeight:700,color:"rgba(255,255,255,0.38)",letterSpacing:"0.06em",textTransform:"uppercase",marginBottom:6}}>Стадия</div>
            ...
            {ORDER_STATUSES.map(s=>{
              ...
              return (
                <button key={s.id} onClick={()=>onUpdate({...order,status:s.id})} style={{
                  ...
                  border:`1.5px solid ${active?s.color+"88":"rgba(255,255,255,0.09)"}`,
          ...
          {order.message && (
            <pre style={{
              background:"#111113", border:"1px solid #1a2540", borderRadius:9,
              padding:"8px 10px", fontSize:11, color:"rgba(255,255,255,0.4)",
              fontFamily:"'DM Sans',-apple-system,sans-serif", whiteSpace:"pre-wrap", margin:0, maxHeight:160, overflowY:"auto",
            }}>{order.message}</pre>
          )}
          {order.nextDate && (
            <div style={{
          ...
          <div style={{display:"flex",gap:7,marginTop:10,alignItems:"center",flexWrap:"wrap"}}>
            <button onClick={handleCopyOrder} style={{
              ...
              color: copiedOrder ? "#34C759" : "#007AFF",
              borderRadius:9, padding:"5px 12px", fontSize:11, fontWeight:700,
              cursor:"pointer", fontFamily:"inherit", transition:"all 0.2s",
              minWidth:110,
            }}>{copiedOrder ? "✓ Скопировано!" : "📋 Копировать"}</button>
            <button onClick={startEdit} style={{
              background:"#1a2a10", border:"1.5px solid #3a6a2a", color:"#6aaa4a",
              borderRadius:9, padding:"5px 12px", fontSize:11, fontWeight:700,
              cursor:"pointer", fontFamily:"inherit",
            }}>✏️ Изменить</button>
            {onRepeat && (
              <button onClick={()=>onRepeat(order)} style={{
                background:"#1a1a30", border:"1.5px solid #5a4aad", color:"#D8B4FE",
                borderRadius:9, padding:"5px 12px", fontSize:11, fontWeight:700,
                cursor:"pointer", fontFamily:"inherit",
              }}>🔄 Повторить</button>
            )}
            <div style={{fontSize:9,color:"rgba(255,255,255,0.15)"}}>{order.id}</div>
            <button
              onClick={()=>{
            ...
            >{confirmDel ? "🗑 Точно?" : "🗑"}</button>
          </div>
        </div>
      )}
      {editMode && (
        <div style={{borderTop:"1px solid #2a4a80",padding:"12px"}}>
          <div style={{fontSize:10,fontWeight:700,color:"#007AFF",letterSpacing:"0.06em",textTransform:"uppercase",marginBottom:10}}>
            ✏️ Редактирование заказа
          </div>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:8}}>
            <div>
              <div style={{fontSize:10,color:"rgba(255,255,255,0.38)",fontWeight:700,marginBottom:4}}>Клиент</div>
              <input value={draft.clientName||""} onChange={e=>setD("clientName",e.target.value)} placeholder="Имя" style={fieldStyle}/>
            </div>
            <div>
              <div style={{fontSize:10,color:"rgba(255,255,255,0.38)",fontWeight:700,marginBottom:4}}>Телефон</div>
              <input value={draft.phone||""} onChange={e=>setD("phone",e.target.value)} placeholder="+7..." style={fieldStyle}/>
            </div>
          </div>
          <div style={{marginBottom:8}}>
            <div style={{fontSize:10,color:"rgba(255,255,255,0.38)",fontWeight:700,marginBottom:4}}>Адрес</div>
            <input value={draft.address||""} onChange={e=>setD("address",e.target.value)} placeholder="Адрес" style={{...fieldStyle}}/>
          </div>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8,marginBottom:8}}>
            <div>
              <div style={{fontSize:10,color:"rgba(255,255,255,0.38)",fontWeight:700,marginBottom:4}}>Дата</div>
              <input type="date" value={draft.date||""} onChange={e=>setD("date",e.target.value)} style={{...fieldStyle,WebkitAppearance:"none"}}/>
            </div>
            <div>
              <div style={{fontSize:10,color:"rgba(255,255,255,0.38)",fontWeight:700,marginBottom:4}}>Сумма (т)</div>
              <input type="number" step="0.1" value={draft.amount||""} onChange={e=>setD("amount",e.target.value)} placeholder="0.0" style={fieldStyle}/>
            </div>
            <div>
              <div style={{fontSize:10,color:"rgba(255,255,255,0.38)",fontWeight:700,marginBottom:4}}>Маржа (т)</div>
          ...
          <div style={{marginBottom:10}}>
            <div style={{fontSize:10,color:"rgba(255,255,255,0.38)",fontWeight:700,marginBottom:4}}>Стадия</div>
          ...
          <div style={{marginBottom:10}}>
            <div style={{fontSize:10,color:"rgba(255,255,255,0.38)",fontWeight:700,marginBottom:4}}>Следующий заказ</div>
          ...
          <div style={{display:"flex",gap:8}}>
            ...
            <button onClick={cancelEdit} style={{
              background:"transparent", border:"1.5px solid #1e2d4a",
              color:"rgba(255,255,255,0.38)", borderRadius:10, padding:"7px 14px",
...
function ClientCard({ client, orders, onUpdateOrder, onDeleteOrder, onRepeat }) {
  ...
  return (
    <div style={{
      background:"#1C1C1E", border:"1px solid #1e2d4a",
      ...
      <div onClick={()=>setExpanded(!expanded)} style={{
        ...
        <div style={{
          ...
          border:"1.5px solid #2a4a80",
          display:"flex", alignItems:"center", justifyContent:"center",
          fontSize:13, fontWeight:800, color:"#007AFF", flexShrink:0,
        }}>{initials}</div>
        ...
        <div style={{flex:1,minWidth:0}}>
          ...
          <div style={{fontSize:11,color:"rgba(255,255,255,0.38)",marginTop:3,display:"flex",gap:10,flexWrap:"wrap"}}>
            {client.phone && <span>📞 {client.phone}</span>}
            {client.address && <span style={{overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",maxWidth:220}}>📍 {client.address}</span>}
          </div>
        </div>

        <div style={{display:"flex",gap:6,alignItems:"center",flexShrink:0}}>
          <div style={{fontSize:10,padding:"3px 8px",borderRadius:5,background:"#8b5cf618",color:"#AF52DE",fontWeight:700}}>
            {orders.length} {orders.length===1?"заказ":orders.length<5?"заказа":"заказов"}
          </div>
          {activeCount>0 && (
            <div style={{fontSize:10,padding:"3px 8px",borderRadius:5,background:"#3b82f618",color:"#007AFF",fontWeight:700}}>
              🔵 {activeCount} активн.
            </div>
          )}
          {totalAmount>0 && (
            <div style={{fontSize:10,padding:"3px 8px",borderRadius:5,background:"#22c55e18",color:"#34C759",fontWeight:700}}>
              {totalAmount.toFixed(1)} т
            </div>
          )}
          <span style={{color:"rgba(255,255,255,0.18)",fontSize:10,marginLeft:4}}>{expanded?"▲":"▼"}</span>
        </div>
      </div>

      {expanded && (
        <div style={{borderTop:"1px solid #1a2540",padding:"10px 12px",background:"#141416"}}>
          {orders.length === 0 ? (
            <div style={{textAlign:"center",padding:"20px 0",color:"rgba(255,255,255,0.28)",fontSize:12}}>
              Нет заказов
            </div>
          ) : (
            orders
          ...
          <div style={{fontSize:10,color:"rgba(255,255,255,0.15)",marginTop:6,display:"flex",gap:14}}>
            <span>Выполнено: {doneCount}</span>
            <span>Последний: {lastDate ? new Date(lastDate).toLocaleDateString("ru-RU") : "—"}</span>
          </div>
        </div>
      )}
    </div>
  ...
  return (
    <div style={{flex:1,overflowY:"auto",padding:"16px 20px"}}>
      ...
      <div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:8,alignItems:"center"}}>
        <span style={{fontSize:10,fontWeight:700,color:"rgba(255,255,255,0.18)",letterSpacing:"0.07em",textTransform:"uppercase",marginRight:2}}>Статус</span>
        <button onClick={()=>setFilter("all")} style={{
          padding:"4px 11px", borderRadius:9,
          border:`1.5px solid ${filter==="all"?"rgba(0,122,255,0.6)":"rgba(255,255,255,0.09)"}`,
          background: filter==="all"?"rgba(0,122,255,0.12)":"transparent",
          color: filter==="all"?"#007AFF":"rgba(255,255,255,0.38)",
          fontSize:11, fontWeight:700, cursor:"pointer", fontFamily:"inherit",
        }}>Все ({orders.length})</button>
        {ORDER_STATUSES.map(s=>counts[s.id]>0&&(
          <button key={s.id} onClick={()=>setFilter(filter===s.id?"all":s.id)} style={{
            padding:"4px 11px", borderRadius:9,
            border:`1.5px solid ${filter===s.id?s.color+"88":"rgba(255,255,255,0.09)"}`,
            background: filter===s.id?s.color+"22":"transparent",
            color: filter===s.id?s.color:"rgba(255,255,255,0.38)",
            fontSize:11, fontWeight:700, cursor:"pointer", fontFamily:"inherit",
        ...
        <input
          ...
          style={{
            marginLeft:"auto", background:"#1C1C1E",
      ...
      <div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:14,alignItems:"center"}}>
        <span style={{fontSize:10,fontWeight:700,color:"rgba(255,255,255,0.18)",letterSpacing:"0.07em",textTransform:"uppercase",marginRight:2}}>Услуга</span>
        <button onClick={()=>setFilterService("all")} style={{
          padding:"4px 11px", borderRadius:9,
          border:`1.5px solid ${filterService==="all"?"rgba(0,122,255,0.6)":"rgba(255,255,255,0.09)"}`,
          background: filterService==="all"?"rgba(0,122,255,0.12)":"transparent",
          color: filterService==="all"?"#007AFF":"rgba(255,255,255,0.38)",
          fontSize:11, fontWeight:700, cursor:"pointer", fontFamily:"inherit",
        }}>Все</button>
        {SERVICES.filter(s=>!s.add).map(s=>(
          <button key={s.id} onClick={()=>setFilterService(filterService===s.id?"all":s.id)} style={{
            padding:"4px 11px", borderRadius:9,
            border:`1.5px solid ${filterService===s.id?s.color+"88":"rgba(255,255,255,0.09)"}`,
            background: filterService===s.id?s.color+"22":"transparent",
            color: filterService===s.id?s.color:"rgba(255,255,255,0.38)",
            fontSize:11, fontWeight:700, cursor:"pointer", fontFamily:"inherit",
          }}>{s.icon} {s.label} {svcCounts[s.id]>0?`(${svcCounts[s.id]})`:""}</button>
        ))}
      </div>

      {/* Clients list */}
      {clients.length === 0 ? (
        <div style={{textAlign:"center",padding:"60px 0",color:"rgba(255,255,255,0.15)",fontSize:14}}>
          {orders.length === 0
...
function ReportView({ orders }) {
  ...
  const [metrics, setMetrics] = useState({ count: true, amount: true, margin: true });
  ...
  return (
    <div style={{ flex: 1, overflowY: "auto", padding: "18px 20px" }}>
      <div style={{ marginBottom: 18 }}>
        <div style={{ fontSize: 20, fontWeight: 800, color: "#FFFFFF", letterSpacing: "-0.02em" }}>Отчёт</div>
        <div style={{ fontSize: 12, color: "rgba(255,255,255,0.28)", marginTop: 2 }}>Сводные данные по услугам за период</div>
      </div>
      ...
      <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginBottom: 14 }}>
        {PERIOD_PRESETS.map(p => (
          <button key={p.id} onClick={() => setPreset(p.id)} style={{
            ...
            border: `1.5px solid ${preset === p.id ? "rgba(0,122,255,0.6)" : "rgba(255,255,255,0.09)"}`,
            background: preset === p.id ? "rgba(0,122,255,0.12)" : "transparent",
            color: preset === p.id ? "#007AFF" : "rgba(255,255,255,0.38)",
      ...
      {preset === "custom" && (
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 14 }}>
          <div>
            <div style={{ fontSize: 10, fontWeight: 700, color: "rgba(255,255,255,0.38)", letterSpacing: "0.07em", textTransform: "uppercase", marginBottom: 5 }}>От</div>
            <input type="date" value={customFrom} onChange={e => setCustomFrom(e.target.value)}
              style={{ width: "100%", background: "#1C1C1E", border: "1.5px solid #1e2d4a", borderRadius: 9, color: "rgba(255,255,255,0.85)", fontSize: 13, padding: "9px 12px", outline: "none", fontFamily: "inherit", boxSizing: "border-box" }}
            />
          </div>
          <div>
            <div style={{ fontSize: 10, fontWeight: 700, color: "rgba(255,255,255,0.38)", letterSpacing: "0.07em", textTransform: "uppercase", marginBottom: 5 }}>До</div>
      ...
      <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginBottom: 18 }}>
        <div style={{ fontSize: 10, fontWeight: 700, color: "rgba(255,255,255,0.28)", letterSpacing: "0.07em", textTransform: "uppercase", alignSelf: "center", marginRight: 4 }}>Показывать:</div>
        {[
          { id: "count",  label: "Заказы",  color: "#007AFF" },
          { id: "amount", label: "Сумма",   color: "#34C759" },
          { id: "margin", label: "Маржа",   color: "#FF9500" },
        ].map(m => (
          <button key={m.id} onClick={() => setMetrics(p => ({ ...p, [m.id]: !p[m.id] }))} style={{
            padding: "4px 10px", borderRadius: 7,
            border: `1.5px solid ${metrics[m.id] ? m.color + "88" : "rgba(255,255,255,0.09)"}`,
            background: metrics[m.id] ? m.color + "22" : "transparent",
            color: metrics[m.id] ? m.color : "rgba(255,255,255,0.38)",
            fontSize: 11, fontWeight: 700, cursor: "pointer", fontFamily: "inherit",
          }}>{metrics[m.id] ? "✓ " : ""}{m.label}</button>
        ))}
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10, marginBottom: 16 }}>
        {metrics.count && (
          <div style={{ background: "#1C1C1E", border: "1px solid #1e2d4a", borderRadius: 12, padding: "14px 16px" }}>
            <div style={{ fontSize: 10, fontWeight: 700, color: "#007AFF", letterSpacing: "0.08em", textTransform: "uppercase" }}>Заказов</div>
            <div style={{ fontSize: 26, fontWeight: 800, color: "#FFFFFF", marginTop: 4, letterSpacing: "-0.02em" }}>{totals.count}</div>
          </div>
        )}
        {metrics.amount && (
          <div style={{ background: "#1C1C1E", border: "1px solid #1e2d4a", borderRadius: 12, padding: "14px 16px" }}>
            <div style={{ fontSize: 10, fontWeight: 700, color: "#34C759", letterSpacing: "0.08em", textTransform: "uppercase" }}>Сумма, т</div>
            <div style={{ fontSize: 26, fontWeight: 800, color: "#FFFFFF", marginTop: 4, letterSpacing: "-0.02em" }}>{fmt(totals.amount)}</div>
          </div>
        )}
        {metrics.margin && (
          <div style={{ background: "#1C1C1E", border: "1px solid #1e2d4a", borderRadius: 12, padding: "14px 16px" }}>
            <div style={{ fontSize: 10, fontWeight: 700, color: "#FF9500", letterSpacing: "0.08em", textTransform: "uppercase" }}>Маржа, т</div>
            <div style={{ fontSize: 26, fontWeight: 800, color: "#FFFFFF", marginTop: 4, letterSpacing: "-0.02em" }}>{fmt(totals.margin)}</div>
          </div>
        )}
      </div>

      {/* By service */}
      <div style={{ fontSize: 11, fontWeight: 700, color: "rgba(255,255,255,0.38)", letterSpacing: "0.08em", textTransform: "uppercase", marginBottom: 10 }}>По услугам</div>

      {Object.entries(agg).map(([id, s]) => (
        <div key={id} style={{
          background: "#1C1C1E", border: `1px solid ${s.color}33`,
          ...
          <div style={{ display: "flex", gap: 14, fontSize: 12 }}>
            {metrics.count && (
              <div style={{ textAlign: "right" }}>
                <div style={{ fontSize: 9, color: "rgba(255,255,255,0.38)", fontWeight: 700, letterSpacing: "0.06em" }}>ЗАКАЗЫ</div>
                <div style={{ color: "#007AFF", fontWeight: 800, fontSize: 15 }}>{s.count}</div>
              </div>
            )}
            {metrics.amount && (
              <div style={{ textAlign: "right" }}>
                <div style={{ fontSize: 9, color: "rgba(255,255,255,0.38)", fontWeight: 700, letterSpacing: "0.06em" }}>СУММА</div>
                <div style={{ color: "#34C759", fontWeight: 800, fontSize: 15 }}>{fmt(s.amount)} т</div>
              </div>
            )}
            {metrics.margin && (
              <div style={{ textAlign: "right" }}>
                <div style={{ fontSize: 9, color: "rgba(255,255,255,0.38)", fontWeight: 700, letterSpacing: "0.06em" }}>МАРЖА</div>
                <div style={{ color: "#FF9500", fontWeight: 800, fontSize: 15 }}>{fmt(s.margin)} т</div>
              </div>
            )}
          </div>
        </div>
      ...
      <div style={{ fontSize: 10, color: "rgba(255,255,255,0.18)", marginTop: 16, textAlign: "center" }}>
        Учётный период: {range.start.toLocaleDateString("ru-RU")} — {range.end.toLocaleDateString("ru-RU")}
        {preset === "month" && <span style={{color:"rgba(255,255,255,0.28)"}}> · неделя начавшаяся в месяце = этот месяц</span>}
      </div>
    </div>
  );
}
...
function AnalyticsView({ orders }) {
  ...
  // Auto compare presets
  const [autoPreset, setAutoPreset] = useState("month"); // week | month | year
  // Custom compare — two free periods
  ...
  const metricColor = { count: "#007AFF", amount: "#34C759", margin: "#FF9500" };
  const pct = (a, b) => b === 0 ? (a > 0 ? 100 : 0) : Math.round(((a - b) / b) * 100);

  // Вычисляем два диапазона в зависимости от режима
  const getRanges = () => {
    const now = new Date();
    if (mode === "auto") {
      if (autoPreset === "week") {
        ...
        const b = prevWeek();
        return [
          { ...a, label: `Эта неделя (${rangeLabel(a)})` },
          { ...b, label: `Прошлая неделя (${rangeLabel(b)})` },
        ];
      }
      if (autoPreset === "month") {
        const a = currentAccountingMonth();
        const b = prevAccountingMonth();
        const mA = RU_MONTHS_FULL[now.getMonth()];
        const mB = RU_MONTHS_FULL[(now.getMonth()-1+12)%12];
        const yB = now.getMonth()===0 ? now.getFullYear()-1 : now.getFullYear();
        return [
          { ...a, label: `${mA} (учётный)` },
          { ...b, label: `${mB} ${now.getFullYear()===yB?"":yB} (учётный)` },
        ];
      }
      if (autoPreset === "year") {
        const y = now.getFullYear();
        const a = accountingYear(y);
        const b = accountingYear(y - 1);
        return [
          { ...a, label: `${y} (учётный)` },
          { ...b, label: `${y-1} (учётный)` },
        ];
    ...
    return [
      { ...parseRange(aFrom, aTo), label: (aFrom && aTo) ? `${aFrom} — ${aTo}` : "Период А" },
      { ...parseRange(bFrom, bTo), label: (bFrom && bTo) ? `${bFrom} — ${bTo}` : "Период Б" },
    ];
  };
  ...
  const inputStyle = { background:"#1C1C1E", border:"1.5px solid #1e2d4a", borderRadius:10, color:"rgba(255,255,255,0.85)", fontSize:12, padding:"8px 11px", outline:"none", fontFamily:"inherit", boxSizing:"border-box", width:"100%", WebkitAppearance:"none", appearance:"none" };
  ...
  return (
    <div style={{ flex:1, overflowY:"auto", padding:"18px 20px" }}>
      <div style={{ marginBottom:16 }}>
        <div style={{ fontSize:20, fontWeight:800, color:"#FFFFFF", letterSpacing:"-0.02em" }}>Аналитика</div>
        <div style={{ fontSize:12, color:"rgba(255,255,255,0.28)", marginTop:2 }}>Сравнение двух периодов</div>
      </div>

      {/* Mode toggle */}
      <div style={{ display:"flex", gap:6, marginBottom:14 }}>
        {[["auto","⚡ Быстрое сравнение"],["custom","📅 Свои периоды"]].map(([m, lbl]) => (
          <button key={m} onClick={()=>setMode(m)} style={{
            padding:"7px 16px", borderRadius:11,
            border:`1.5px solid ${mode===m?"rgba(0,122,255,0.6)":"rgba(255,255,255,0.09)"}`,
            background: mode===m?"rgba(0,122,255,0.12)":"transparent",
            color: mode===m?"#007AFF":"rgba(255,255,255,0.38)",
            fontSize:12, fontWeight:700, cursor:"pointer", fontFamily:"inherit",
          }}>{lbl}</button>
        ))}
      </div>
      ...
      {mode === "auto" && (
        <div style={{ display:"flex", gap:6, marginBottom:14, flexWrap:"wrap" }}>
          {[["week","Эта неделя vs прошлая"],["month","Этот месяц vs прошлый"],["year","Этот год vs прошлый"]].map(([p,lbl])=>(
            <button key={p} onClick={()=>setAutoPreset(p)} style={{
              padding:"7px 14px", borderRadius:10,
              border:`1.5px solid ${autoPreset===p?"#8b5cf688":"rgba(255,255,255,0.09)"}`,
              background: autoPreset===p?"#8b5cf622":"transparent",
              color: autoPreset===p?"#D8B4FE":"rgba(255,255,255,0.38)",
              fontSize:12, fontWeight:700, cursor:"pointer", fontFamily:"inherit",
            }}>{lbl}</button>
          ))}
        </div>
      ...
      {mode === "custom" && (
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12, marginBottom:14 }}>
          <div style={{ background:"#1C1C1E", border:`1px solid ${colorA}33`, borderRadius:14, padding:"12px 14px" }}>
            <div style={{ fontSize:11, fontWeight:700, color:colorA, letterSpacing:"0.08em", textTransform:"uppercase", marginBottom:10 }}>
              🔵 Период А
            </div>
            <div style={{ display:"flex", flexDirection:"column", gap:7 }}>
              <input type="date" value={aFrom} onChange={e=>setAFrom(e.target.value)} style={inputStyle} placeholder="От"/>
              <input type="date" value={aTo}   onChange={e=>setATo(e.target.value)}   style={inputStyle} placeholder="До"/>
            </div>
          </div>
          <div style={{ background:"#1C1C1E", border:`1px solid ${colorB}33`, borderRadius:14, padding:"12px 14px" }}>
            <div style={{ fontSize:11, fontWeight:700, color:colorB, letterSpacing:"0.08em", textTransform:"uppercase", marginBottom:10 }}>
              🟠 Период Б
            </div>
            <div style={{ display:"flex", flexDirection:"column", gap:7 }}>
              <input type="date" value={bFrom} onChange={e=>setBFrom(e.target.value)} style={inputStyle} placeholder="От"/>
              <input type="date" value={bTo}   onChange={e=>setBTo(e.target.value)}   style={inputStyle} placeholder="До"/>
            </div>
          </div>
        </div>
      )}
      ...
      {/* Period labels */}
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10, marginBottom:14 }}>
        <div style={{ background:`${colorA}11`, border:`1px solid ${colorA}33`, borderRadius:11, padding:"8px 12px", fontSize:12, color:colorA, fontWeight:700, textAlign:"center" }}>
          🔵 {rangeA.label}
        </div>
        <div style={{ background:`${colorB}11`, border:`1px solid ${colorB}33`, borderRadius:11, padding:"8px 12px", fontSize:12, color:colorB, fontWeight:700, textAlign:"center" }}>
          🟠 {rangeB.label}
        </div>
      </div>
      ...
      <div style={{ display:"flex", gap:6, flexWrap:"wrap", marginBottom:18, alignItems:"center" }}>
        <span style={{ fontSize:10, fontWeight:700, color:"rgba(255,255,255,0.28)", letterSpacing:"0.07em", textTransform:"uppercase", marginRight:4 }}>Метрика:</span>
        {["count","amount","margin"].map(m=>(
          <button key={m} onClick={()=>setMetric(m)} style={{
            padding:"5px 12px", borderRadius:9,
            border:`1.5px solid ${metric===m?metricColor[m]+"88":"rgba(255,255,255,0.09)"}`,
            background: metric===m?metricColor[m]+"22":"transparent",
            color: metric===m?metricColor[m]:"rgba(255,255,255,0.38)",
            fontSize:11, fontWeight:700, cursor:"pointer", fontFamily:"inherit",
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:10, marginBottom:20 }}>
        {[
          { id:"count", label:"Заказы",  color:"#007AFF", suffix:"" },
          { id:"amount",label:"Сумма",   color:"#34C759", suffix:" т" },
          { id:"margin",label:"Маржа",   color:"#FF9500", suffix:" т" },
        ].map(m => {
          const va = totA[m.id], vb = totB[m.id];
          const diff = va - vb;
          const p = pct(va, vb);
          const up = diff >= 0;
          return (
            <div key={m.id} style={{ background:"#1C1C1E", border:"1px solid #1e2d4a", borderRadius:14, padding:"12px 14px" }}>
              <div style={{ fontSize:10, fontWeight:700, color:m.color, letterSpacing:"0.08em", textTransform:"uppercase", marginBottom:6 }}>{m.label}</div>
              {/* A */}
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"baseline", marginBottom:3 }}>
                <span style={{ fontSize:10, color:colorA, fontWeight:700 }}>🔵</span>
                <span style={{ fontSize:18, fontWeight:800, color:"#FFFFFF", letterSpacing:"-0.02em" }}>{fmt(va)}{m.suffix}</span>
              </div>
              {/* B */}
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"baseline", marginBottom:8 }}>
                <span style={{ fontSize:10, color:colorB, fontWeight:700 }}>🟠</span>
                <span style={{ fontSize:13, fontWeight:600, color:"rgba(255,255,255,0.35)" }}>{fmt(vb)}{m.suffix}</span>
              </div>
              {/* Delta */}
              <div style={{ fontSize:11, fontWeight:800, color: up?"#34C759":"#FF3B30", borderTop:"1px solid #1a2540", paddingTop:6, textAlign:"center" }}>
                {up?"▲":"▼"} {Math.abs(p)}%
                <span style={{ fontSize:10, color:"rgba(255,255,255,0.38)", fontWeight:500, marginLeft:5 }}>
                  ({up?"+":""}{fmt(diff)}{m.suffix})
                </span>
              </div>
            </div>
          );
      ...
      <div style={{ fontSize:11, fontWeight:700, color:"rgba(255,255,255,0.38)", letterSpacing:"0.08em", textTransform:"uppercase", marginBottom:10 }}>
        По услугам — {metricLabel[metric]}
      </div>

      <div style={{ background:"#1C1C1E", border:"1px solid #1e2d4a", borderRadius:14, padding:"16px", marginBottom:20 }}>
        {SERVICES.filter(s=>!s.add).map(s => {
          ...
          return (
            <div key={s.id} style={{ marginBottom:14 }}>
              <div style={{ display:"flex", justifyContent:"space-between", marginBottom:5 }}>
                <div style={{ fontSize:12, color:"rgba(255,255,255,0.85)", fontWeight:600 }}>{s.icon} {s.label}</div>
              ...
              {/* Bar A */}
              <div style={{ height:7, background:"#1C1C1E", borderRadius:4, overflow:"hidden", marginBottom:3 }}>
                <div style={{ width:`${(va/maxV)*100}%`, height:"100%", background:colorA, transition:"width 0.4s ease", borderRadius:4 }}/>
              </div>
              {/* Bar B */}
              <div style={{ height:7, background:"#1C1C1E", borderRadius:4, overflow:"hidden" }}>
                <div style={{ width:`${(vb/maxV)*100}%`, height:"100%", background:colorB, opacity:0.8, transition:"width 0.4s ease", borderRadius:4 }}/>
              </div>
            </div>
          );
        })}
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10, marginBottom:16 }}>
        {[
          { range: rangeA, ord: ordA, color: colorA, label: rangeA.label },
          { range: rangeB, ord: ordB, color: colorB, label: rangeB.label },
        ].map(({ ord, color, label }) => {
          const stDist = ORDER_STATUSES.map(s=>({ ...s, count: ord.filter(o=>o.status===s.id).length }));
          const total = ord.length || 1;
          return (
            <div key={label} style={{ background:"#1C1C1E", border:`1px solid ${color}33`, borderRadius:14, padding:"12px 14px" }}>
              <div style={{ fontSize:11, fontWeight:700, color, marginBottom:8 }}>
                {color===colorA?"🔵":"🟠"} {label} <span style={{ color:"rgba(255,255,255,0.38)", fontWeight:500 }}>({ord.length} заказ.)</span>
              </div>
              <div style={{ height:8, display:"flex", borderRadius:4, overflow:"hidden", marginBottom:8 }}>
                {stDist.map(s => s.count>0 && (
                  <div key={s.id} style={{ width:`${(s.count/total)*100}%`, background:s.color }} title={`${s.label}: ${s.count}`}/>
                ))}
              </div>
              <div style={{ display:"flex", flexWrap:"wrap", gap:6 }}>
                {stDist.filter(s=>s.count>0).map(s=>(
                  <span key={s.id} style={{ fontSize:10, color:s.color, fontWeight:700 }}>
                    {s.label}: {s.count}
                  </span>
                ))}
              </div>
      ...
      {ordA.length === 0 && ordB.length === 0 && (
        <div style={{ textAlign:"center", padding:"30px 0", color:"rgba(255,255,255,0.18)", fontSize:13 }}>
    ...
    <div style={{ flex: 1, overflowY: "auto", padding: "18px 20px" }}>
      ...
      <div style={{ display: "flex", gap: 6, marginBottom: 18 }}>
        {[["payments","💳 Долги и платежи"],["expenses","📤 Расходы"]].map(([tab, label]) => (
          <button key={tab} onClick={() => setActiveTab(tab)} style={{
            ...
            border: `1.5px solid ${activeTab === tab ? "rgba(0,122,255,0.6)" : "rgba(255,255,255,0.09)"}`,
            background: activeTab === tab ? "rgba(0,122,255,0.14)" : "transparent",
            color: activeTab === tab ? "#007AFF" : "rgba(255,255,255,0.38)",
      ...
      <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginBottom: 14, alignItems: "center" }}>
        <button onClick={() => setSelectedMonth("all")} style={{
          padding: "6px 14px", borderRadius: 8,
          border: `1.5px solid ${selectedMonth === "all" ? "rgba(0,122,255,0.6)" : "rgba(255,255,255,0.09)"}`,
          background: selectedMonth === "all" ? "rgba(0,122,255,0.12)" : "transparent",
          color: selectedMonth === "all" ? "#007AFF" : "rgba(255,255,255,0.38)",
          fontSize: 12, fontWeight: 700, cursor: "pointer", fontFamily: "inherit",
        }}>Всё время</button>
        {allMonths.map(m => (
          <button key={m} onClick={() => setSelectedMonth(m)} style={{
            padding: "6px 14px", borderRadius: 8,
            border: `1.5px solid ${selectedMonth === m ? "rgba(0,122,255,0.6)" : "rgba(255,255,255,0.09)"}`,
            background: selectedMonth === m ? "rgba(0,122,255,0.12)" : "transparent",
            color: selectedMonth === m ? "#007AFF" : "rgba(255,255,255,0.38)",
            fontSize: 12, fontWeight: 700, cursor: "pointer", fontFamily: "inherit",
          }}>{monthLabel(m)}</button>
        ))}
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10, marginBottom: 18 }}>
        <div style={{ background: "#1C1C1E", border: "1px solid #1e2d4a", borderRadius: 12, padding: "14px 16px" }}>
          ...
          <div style={{ fontSize: 26, fontWeight: 800, color: "#FFFFFF", marginTop: 4, letterSpacing: "-0.02em" }}>
            {fmtT(displayEarned)} <span style={{ fontSize: 16, color: "rgba(255,255,255,0.4)" }}>т</span>
          </div>
        </div>
        <div style={{ background: "#1C1C1E", border: "1px solid #22c55e33", borderRadius: 12, padding: "14px 16px" }}>
          <div style={{ fontSize: 10, fontWeight: 700, color: "#34C759", letterSpacing: "0.08em", textTransform: "uppercase" }}>Получено</div>
          <div style={{ fontSize: 26, fontWeight: 800, color: "#FFFFFF", marginTop: 4, letterSpacing: "-0.02em" }}>
            {fmtT(displayPaid)} <span style={{ fontSize: 16, color: "rgba(255,255,255,0.4)" }}>т</span>
          </div>
        </div>
        <div style={{ background: "#1C1C1E", border: displayDebt > 0.01 ? "1px solid #ef444455" : "1px solid #22c55e33", borderRadius: 12, padding: "14px 16px" }}>
          ...
          <div style={{ fontSize: 26, fontWeight: 800, color: displayDebt > 0.01 ? "#FF3B30" : "#34C759", marginTop: 4, letterSpacing: "-0.02em" }}>
            ...
            {displayDebt > 0.01 && <span style={{ fontSize: 16, color: "rgba(255,255,255,0.4)" }}> т</span>}
          </div>
        </div>
      </div>

      {/* Add Payment */}
      <div style={{ background: "rgba(52,199,89,0.08)", border: "1px solid #22c55e33", borderRadius: 12, padding: "14px 16px", marginBottom: 12 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: 8 }}>
          <input type="number" step="0.1" placeholder="Сумма в тоннах"
            ...
            style={{
              background: "#1C1C1E", border: "1.5px solid #1e2d4a",
        ...
        <div style={{ display: "flex", gap: 8 }}>
          <input type="text" placeholder="Комментарий (необязательно)"
            ...
            style={{
              flex: 1, background: "#1C1C1E", border: "1.5px solid #1e2d4a",
              borderRadius: 8, color: "rgba(255,255,255,0.85)", fontSize: 13,
              padding: "9px 12px", outline: "none", fontFamily: "inherit",
              boxSizing: "border-box",
            }}
          ...
          <button onClick={handleAddPayment} disabled={!payAmount} style={{
            background: payAmount ? "rgba(52,199,89,0.12)" : "#111113",
            border: `1.5px solid ${payAmount ? "#22c55e88" : "rgba(255,255,255,0.07)"}`,
            color: payAmount ? "#34C759" : "rgba(255,255,255,0.28)",
      ...
      <div style={{ marginBottom: 12 }}>
        <button onClick={() => setShowManual(!showManual)} style={{
          ...
          border: `1.5px solid ${showManual ? "#ef444488" : "rgba(255,255,255,0.09)"}`,
          color: showManual ? "#FF3B30" : "rgba(255,255,255,0.38)",
        ...
        <div style={{ background: "rgba(255,59,48,0.08)", border: "1px solid #ef444433", borderRadius: 12, padding: "14px 16px", marginBottom: 14 }}>
          ...
          <div style={{ display: "flex", gap: 8 }}>
            ...
            <button onClick={handleAddManual} disabled={!manualAmount} style={{
              background: manualAmount ? "rgba(255,59,48,0.12)" : "#111113",
              border: `1.5px solid ${manualAmount ? "#ef444488" : "rgba(255,255,255,0.07)"}`,
              color: manualAmount ? "#FF3B30" : "rgba(255,255,255,0.28)",
              borderRadius: 8, padding: "9px 20px",
              fontSize: 13, fontWeight: 800,
      ...
      {allMonths.map(m => {
        ...
        return (
          <div key={m} style={{
            ...
            {/* Progress bar */}
            <div style={{ height: 6, background: "#111113", borderRadius: 3, overflow: "hidden", marginBottom: 10 }}>
              <div style={{
      ...
      {payments.length > 0 && (
        <>
          <div style={{ fontSize: 11, fontWeight: 700, color: "rgba(255,255,255,0.38)", letterSpacing: "0.08em", textTransform: "uppercase", marginTop: 20, marginBottom: 10 }}>
            История оплат ({payments.length})
          </div>
          <div style={{ background: "#1C1C1E", border: "1px solid #1e2d4a", borderRadius: 12, padding: "6px" }}>
            {[...payments].sort((a,b) => (b.date||"").localeCompare(a.date||"")).map(p => (
              <div key={p.id} style={{
                ...
                <div style={{ fontSize: 12 }}>
                  <span style={{ color: "rgba(255,255,255,0.38)" }}>{fmtDateShort(p.date)}</span>
                  {p.note && <span style={{ color: "rgba(255,255,255,0.28)", marginLeft: 8 }}>· {p.note}</span>}
                </div>
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  ...
                  >{confirmDelId === p.id ? "Точно?" : "🗑"}</button>
                </div>
              </div>
            ))}
          </div>
          ...
          <div style={{ background: "#1C1C1E", border: "1px solid #1e2d4a", borderRadius: 12, padding: "6px" }}>
            {[...manualDebts].sort((a,b) => (b.month||"").localeCompare(a.month||"")).map(d => (
              <div key={d.id} style={{
                ...
                <div style={{ fontSize: 12 }}>
                  <span style={{ color: "rgba(255,255,255,0.38)" }}>{monthLabel(d.month)}</span>
                  {d.note && <span style={{ color: "rgba(255,255,255,0.28)", marginLeft: 8 }}>· {d.note}</span>}
                </div>
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  ...
                  >{confirmDelId === d.id ? "Точно?" : "🗑"}</button>
                </div>
              </div>
            ))}
          </div>
      ...
      {activeTab === "expenses" && (
        <div>
          ...
          <div style={{ background: "rgba(255,149,0,0.08)", border: "1px solid rgba(255,149,0,0.25)", borderRadius: 12, padding: "14px 16px", marginBottom: 16 }}>
            ...
            <div style={{ display: "flex", gap: 8 }}>
              ...
              <button onClick={handleAddExpense} disabled={!expAmount} style={{
                background: expAmount ? "rgba(255,149,0,0.12)" : "#111113",
                border: `1.5px solid ${expAmount ? "rgba(255,149,0,0.5)" : "rgba(255,255,255,0.07)"}`,
                color: expAmount ? "#FF9500" : "rgba(255,255,255,0.28)",
                borderRadius: 8, padding: "9px 20px",
                fontSize: 13, fontWeight: 800,
          ...
          {/* Total */}
          {expenses.length > 0 && (
            <div style={{ background: "#1C1C1E", border: "1px solid rgba(255,149,0,0.2)", borderRadius: 12, padding: "14px 16px", marginBottom: 14 }}>
              ...
              <div style={{ fontSize: 28, fontWeight: 800, color: "#FF9500", letterSpacing: "-0.02em" }}>
                ...
                <span style={{ fontSize: 16, color: "rgba(255,255,255,0.4)", marginLeft: 6 }}>₽</span>
              </div>
            </div>
          )}
                ...
                <div key={exp.id} style={{
                  ...
                  <div>
                    <div style={{ fontSize: 13, fontWeight: 600, color: "rgba(255,255,255,0.85)" }}>
                      {exp.note || <span style={{ color: "rgba(255,255,255,0.28)", fontStyle: "italic" }}>Без назначения</span>}
                    </div>
                    <div style={{ fontSize: 11, color: "rgba(255,255,255,0.28)", marginTop: 2 }}>{fmtDateShort(exp.date)}</div>
                  </div>
                  <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                    ...
                    >{confirmDelExpId === exp.id ? "Точно?" : "🗑"}</button>
                  </div>
                </div>
              ))}
            </div>
...
function DispatchApp() {
  ...
  const [activeView, setActiveView] = useState("orders"); // "form" | "orders"
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <button
          ...
          style={{
            ...
            border: "1px solid " + (showSettings ? "rgba(0,122,255,0.6)" : "rgba(255,255,255,0.09)"),
            color: showSettings ? "#007AFF" : "rgba(255,255,255,0.4)",
      ...
      {showSettings && (
        <div style={{
          background: "#141416", borderBottom: "1px solid #1a2540",
          ...
          <div>
            ...
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center" }}>
              ...
              <label style={{
                ...
                color: "#007AFF", borderRadius: 9, padding: "8px 18px",
                fontSize: 12, fontWeight: 800, cursor: "pointer", fontFamily: "inherit",
                display: "flex", alignItems: "center", gap: 6,
              }}>
                ⬆ Импорт данных
            ...
            <div style={{ fontSize: 10, color: "rgba(255,255,255,0.15)", marginTop: 8 }}>
              Файл сохраняется как <code style={{color:"rgba(255,255,255,0.38)"}}>dispatcher_backup_ДАТА.json</code> — храни в iCloud или Google Drive.
            </div>
          </div>
        </div>
      )}
      ...
      <div style={{
        ...
        {[
          { id: "form",      icon: "➕", label: "Новый заказ", color: "#007AFF" },
          { id: "orders",    icon: "📦", label: "Заказы",      color: "#AF52DE", badge: orders.length },
          { id: "debts",     icon: "💰", label: "Платежи",     color: "#FF3B30" },
          { id: "report",    icon: "🗂️", label: "Отчёт",       color: "#34C759" },
          { id: "analytics", icon: "📊", label: "Аналитика",   color: "#FF9500" },
        ].map(t => {
          ...
          {SERVICES.map(svc => {
            if (svc.add) return (
              <button key="add" style={{
                ...
                color: "rgba(255,255,255,0.28)", fontSize: 16, fontWeight: 700,
                flexShrink: 0,
            ...
            return (
              <button
                ...
                style={{
                  ...
                  color: active ? svc.color : "rgba(255,255,255,0.38)",
      ...
      <div style={{
        ...
        <div style={{
          overflowY: "auto",
          padding: "16px 20px",
          borderRight: showPreview ? "1px solid #1a2540" : "none",
        }}>
          <div style={{
            ...
            <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
              <button
                ...
                style={{
                  background: showParser ? "#1a2a10" : "transparent",
                  border: `1px solid ${showParser ? "#3a6a2a" : "rgba(255,255,255,0.09)"}`,
                  color: showParser ? "#6aaa4a" : "rgba(255,255,255,0.38)",
              ...
              <button
                ...
                style={{
                  background: showPreview ? "rgba(0,122,255,0.12)" : "#141416",
                  border: `1px solid ${showPreview ? "rgba(0,122,255,0.6)" : "rgba(255,255,255,0.09)"}`,
                  color: showPreview ? "#007AFF" : "rgba(255,255,255,0.38)",
                  borderRadius: 7, padding: "4px 10px",
                  fontSize: 11, fontWeight: 600, cursor: "pointer",
                  fontFamily: "inherit", display: "flex", alignItems: "center", gap: 5,
                  transition: "all 0.15s",
          ...
          {showParser && (
            <div style={{
              ...
              <textarea
                ...
                style={{
                  width: "100%", background: "#1C1C1E",
                  border: "1.5px solid #1e2d4a", borderRadius: 9,
              ...
              <button
                ...
                style={{
                  ...
                  background: parsing ? "#111113" : "rgba(52,199,89,0.12)",
                  border: `1.5px solid ${parsing ? "rgba(255,255,255,0.07)" : "#22c55e88"}`,
                  color: parsing ? "rgba(255,255,255,0.28)" : "#34C759",
          ...
          {!showPreview && (
            <div style={{ marginTop: 20, display: "flex", flexDirection: "column", gap: 10 }}>
              <button
                ...
                disabled={!message}
                style={{
                  background: message ? "rgba(0,122,255,0.12)" : "#141416",
                  border: `1.5px solid ${message ? "rgba(0,122,255,0.6)" : "rgba(255,255,255,0.07)"}`,
                  color: message ? "#007AFF" : "rgba(255,255,255,0.28)",
              ...
              <button
                ...
                disabled={!message}
                style={{
                  background: orderSaved ? "rgba(52,199,89,0.12)" : (message ? "rgba(175,82,222,0.12)" : "#141416"),
                  border: `1.5px solid ${orderSaved ? "#22c55e55" : (message ? "#8b5cf655" : "rgba(255,255,255,0.07)")}`,
                  color: orderSaved ? "#34C759" : (message ? "#D8B4FE" : "rgba(255,255,255,0.28)"),
                  borderRadius: 10, padding: "12px",
                  cursor: message ? "pointer" : "not-allowed",
                  fontSize: 13, fontWeight: 700,
              ...
              >
                {orderSaved ? "✓ Заказ сохранён" : "💾 Сохранить заказ"}
              </button>
            </div>
          )}
        </div>
        ...
        <div style={{
          ...
          <button
            ...
            style={{
              background: calStatus === "ok" ? "rgba(52,199,89,0.12)" : calStatus === "err" ? "rgba(255,59,48,0.12)" : "#141416",
              border: `1.5px solid ${calStatus === "ok" ? "#22c55e55" : calStatus === "err" ? "#ef444455" : "rgba(255,255,255,0.07)"}`,
              color: calStatus === "ok" ? "#34C759" : calStatus === "err" ? "#FF3B30" : (f.date ? "rgba(255,255,255,0.45)" : "rgba(255,255,255,0.15)"),
              borderRadius: 10, padding: "10px",
          ...
          <button
            ...
            style={{
              background: sheetsStatus === "ok" ? "rgba(52,199,89,0.12)" : sheetsStatus === "err" ? "rgba(255,59,48,0.12)" : "#141416",
              border: `1.5px solid ${sheetsStatus === "ok" ? "#22c55e55" : sheetsStatus === "err" ? "#ef444455" : "rgba(255,255,255,0.07)"}`,
              color: sheetsStatus === "ok" ? "#34C759" : sheetsStatus === "err" ? "#FF3B30" : (message ? "rgba(255,255,255,0.45)" : "rgba(255,255,255,0.15)"),
              borderRadius: 10, padding: "10px",
          ...
          {f.date && (
            <div style={{
              ...
              <div><span style={{ color: "rgba(255,255,255,0.45)" }}>📅</span> {formatDate(f.date)}</div>
              <div><span style={{ color: "rgba(255,255,255,0.45)" }}>🕐</span> {formatTimeSlot(f.hour)}</div>
              {f.address && <div><span style={{ color: "rgba(255,255,255,0.45)" }}>📍</span> {f.address.slice(0, 40)}{f.address.length > 40 ? "…" : ""}</div>}
            </div>
          )}
        </div>
        )}
...
// ── Data
const LEAD_SERVICES = [
  { id:"trash",   label:"Вывоз мусора", icon:"🗑️", color:"#FF6B35" },
  { id:"sawdust", label:"Опилки",       icon:"🪵", color:"#FFB800" },
  { id:"offcuts", label:"Обрезки доски",icon:"🪚", color:"#30A96F" },
  { id:"other",   label:"Другое",       icon:"❓", color:"#6b7a99" },
];
const SOURCES = [
...
const css = {
  input: {
    width:"100%", background:"rgba(255,255,255,0.09)", border:"1.5px solid #232f45",
...
function DateHourPicker({ value, onChange }) {
  ...
  const sel = {
    background:"rgba(255,255,255,0.09)", border:"1.5px solid #232f45",
...
function Chip({ active, color="#007AFF", onClick, children }) {
  return (
...
function ActionBtn({ onClick, disabled, color="#007AFF", children }) {
  return (
    <button onClick={onClick} disabled={disabled} style={{
      background: color+"22", border:`1.5px solid ${color}44`, color,
      borderRadius:11, padding:"8px 16px", fontSize:12, fontWeight:700,
  ...
  return (
    <div style={{ padding:"20px 18px" }}>
      <div style={{ marginBottom:22 }}>
        ...
        <div style={{ fontSize:13, color:"rgba(255,255,255,0.28)", marginTop:3 }}>Не потеряй контакт — запиши за 10 секунд</div>
      </div>
      ...
      <div style={{ marginBottom:12 }}>
        ...
        <div style={{ display:"flex", gap:7, flexWrap:"wrap" }}>
          {SOURCES.map(s=>(
            <Chip key={s.id} active={f.source===s.id} color="#007AFF"
      ...
      <div style={{ marginBottom:12 }}>
        ...
        <div style={{ display:"flex", gap:7, flexWrap:"wrap" }}>
          {[
            ...
            <Chip key={val} active={f.timing===val} color="#007AFF"
      ...
      <div style={{ marginBottom:12 }}>
        ...
        <input
          ...
          style={{
            width:"100%", background:"rgba(255,255,255,0.09)", border:"1.5px solid #232f45",
            borderRadius:12, color: f.callDate?"rgba(255,255,255,0.85)":"rgba(255,255,255,0.4)",
...
function LeadRow({ lead, onUpdate, onDelete, sheetsUrl }) {
  ...
  return (
    <div style={{
      background:"#111a2e",
      border:`1px solid ${open?"#1e3360":"#141e30"}`,
      ...
      <div onClick={()=>setOpen(!open)} style={{
        ...
        <div style={{flex:1,minWidth:0}}>
          <div style={{fontSize:14,fontWeight:700,color:"rgba(255,255,255,0.85)",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>
            {lead.clientName||<span style={{color:"rgba(255,255,255,0.18)"}}>Без имени</span>}
          </div>
          <div style={{fontSize:11,color:"rgba(255,255,255,0.28)",marginTop:1}}>{lead.phone||"—"}</div>
        </div>
        ...
        {lead.service && (
          <div style={{
            ...
            background: (LEAD_SERVICES.find(s=>s.id===lead.service)?.color||"#007AFF")+"18",
            color: LEAD_SERVICES.find(s=>s.id===lead.service)?.color||"#007AFF",
            fontWeight:600, whiteSpace:"nowrap",
          }}>
            {LEAD_SERVICES.find(s=>s.id===lead.service)?.icon} {LEAD_SERVICES.find(s=>s.id===lead.service)?.label}
          </div>
        )}
        ...
        {/* Arrow */}
        <div style={{color:"rgba(255,255,255,0.18)",fontSize:10}}>{open?"▲":"▼"}</div>
      </div>
        ...
        <div style={{borderTop:"1px solid #0f1c2e", padding:"14px 14px 16px"}}>
          ...
          <div style={{display:"grid", gridTemplateColumns:"1fr 1fr", gap:10, marginBottom:10}}>
            ...
            <div>
              ...
              <div style={{display:"flex", gap:5, flexWrap:"wrap"}}>
                {SOURCES.map(s=>(
                  <Chip key={s.id} active={lead.source===s.id} color="#007AFF"
          ...
          <div style={{marginBottom:10}}>
            ...
            <div style={{display:"flex", gap:6, flexWrap:"wrap"}}>
              {[
                ...
                <Chip key={val} active={lead.timing===val} color="#007AFF"
          ...
          <div style={{marginBottom:10}}>
            ...
            <div style={{display:"flex", gap:8, alignItems:"center"}}>
              <input
                ...
                onChange={e => set("callDate", e.target.value)}
                style={{
                  flex:1, background:"rgba(255,255,255,0.09)", border:"1.5px solid #232f45",
                  borderRadius:12, color: lead.callDate ? "rgba(255,255,255,0.85)" : "rgba(255,255,255,0.35)",
                  fontSize:14, padding:"11px 13px",
                  outline:"none", fontFamily:"inherit", boxSizing:"border-box",
                }}
                onFocus={e=>e.target.style.borderColor="#3a6aaa"}
                onBlur={e=>e.target.style.borderColor="#232f45"}
              />
              {lead.callDate && (
                <button
          ...
          <div style={{display:"flex", gap:8, flexWrap:"wrap", alignItems:"center"}}>
            <button onClick={handleMakeOrder} style={{
              ...
              color:"#34C759", borderRadius:11, padding:"8px 16px",
              fontSize:13, fontWeight:800, cursor:"pointer", fontFamily:"inherit",
              display:"flex", alignItems:"center", gap:6,
            }}>🚀 Оформить заказ</button>
            <ActionBtn onClick={handleSheets} disabled={!sheetsUrl||sheetSt==="loading"}
              color={lead.savedToSheets?"#34C759":sheetSt==="ok"?"#34C759":sheetSt==="err"?"#FF3B30":"#007AFF"}>
...
function ListTab({ leads, onUpdate, onDelete, sheetsUrl }) {
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",flex:1,minHeight:0}}>
      {/* Status filter row */}
      <div style={{padding:"10px 18px 8px",display:"flex",gap:6,flexWrap:"wrap",alignItems:"center",borderBottom:"1px solid #0a1220"}}>
        <span style={{fontSize:10,fontWeight:700,color:"rgba(255,255,255,0.18)",letterSpacing:"0.07em",textTransform:"uppercase",marginRight:2}}>Статус</span>
        <Chip active={filterSt==="all"} color="#007AFF" onClick={()=>setFilterSt("all")}>Все ({leads.length})</Chip>
        ...
        <input placeholder="🔍 Поиск..." value={search} onChange={e=>setSearch(e.target.value)}
          style={{marginLeft:"auto",background:"rgba(255,255,255,0.09)",border:"1px solid #232f45",
            borderRadius:10,color:"rgba(255,255,255,0.85)",fontSize:12,padding:"6px 12px",
            outline:"none",fontFamily:"inherit",width:170}}/>
      </div>

      {/* Service filter row */}
      <div style={{padding:"8px 18px 10px",display:"flex",gap:6,flexWrap:"wrap",alignItems:"center",borderBottom:"1px solid #0f1828"}}>
        <span style={{fontSize:10,fontWeight:700,color:"rgba(255,255,255,0.18)",letterSpacing:"0.07em",textTransform:"uppercase",marginRight:2}}>Услуга</span>
        <Chip active={filterSvc==="all"} color="#6b7a99" onClick={()=>setFilterSvc("all")}>Все</Chip>
        {LEAD_SERVICES.map(s=>(
          <Chip key={s.id} active={filterSvc===s.id} color={s.color}
            onClick={()=>setFilterSvc(filterSvc===s.id?"all":s.id)}>
            {s.icon} {s.label} {svcCounts[s.id]>0?`(${svcCounts[s.id]})` : ""}
          </Chip>
        ))}
      </div>
      ...
      <div style={{flex:1,overflowY:"auto",padding:"10px 18px",display:"flex",flexDirection:"column",gap:6}}>
        {filtered.length===0 && (
          <div style={{textAlign:"center",padding:"50px 0",color:"rgba(255,255,255,0.15)",fontSize:14}}>
...
function doPost(e) {
  ...
  return (
    <div style={{padding:"20px 18px",overflowY:"auto",flex:1}}>
      ...
      <div style={{fontSize:13,color:"rgba(255,255,255,0.28)",marginBottom:24}}>Подключение Google Sheets</div>

      {/* Steps */}
      {[
        {n:1,t:"Откройте Apps Script",d:<>Перейдите на <span style={{color:"#007AFF"}}>script.google.com</span> → «Новый проект»</>},
        ...
        <div key={s.n} style={{display:"flex",gap:14,marginBottom:18}}>
          ...
          <div>
            ...
            <div style={{fontSize:13,color:"rgba(255,255,255,0.38)",lineHeight:1.5}}>{s.d}</div>
          </div>
        </div>
      ))}
      ...
      <div style={{position:"relative",marginBottom:20}}>
        <pre style={{
          background:"#111113",border:"1px solid #1a2a40",borderRadius:12,
        ...
        <button onClick={()=>{copyToClipboard(SCRIPT);setCopied(true);setTimeout(()=>setCopied(false),2000);}}
          style={{
            ...
            color:"#007AFF",borderRadius:9,padding:"4px 12px",
            fontSize:11,fontWeight:700,cursor:"pointer",fontFamily:"inherit",
      ...
      <div>
        ...
        {sheetsUrl && (
          <div style={{marginTop:8,display:"flex",alignItems:"center",gap:6}}>
            <div style={{width:7,height:7,borderRadius:"50%",background:"#34C759",boxShadow:"0 0 6px #22c55e88"}}/>
      ...
      <div style={{marginTop:24}}>
        ...
        <div style={{display:"flex",gap:8,flexWrap:"wrap",alignItems:"center"}}>
          ...
          <label style={{
            background:"rgba(0,122,255,0.12)", border:"1.5px solid #2a5aad",
            color:"#007AFF", borderRadius:11, padding:"8px 18px",
            fontSize:12, fontWeight:800, cursor:"pointer", fontFamily:"inherit",
            display:"flex", alignItems:"center", gap:6,
          }}>
            ⬆ Импорт лидов
        ...
        <div style={{fontSize:10,color:"rgba(255,255,255,0.15)",marginTop:8}}>
          Файл сохраняется как <code style={{color:"rgba(255,255,255,0.38)"}}>leads_backup_ДАТА.json</code>
        </div>
      </div>
    </div>
  );
        ...
        return (
          <div
            ...
            style={{
              ...
              borderRadius:14,
              border: `1.5px solid ${isOver ? col.color+"55" : "#111e35"}`,
              transition:"all 0.15s",
            }}
          >
            {/* Column header */}
            ...
            <div style={{
              ...
              {colLeads.length === 0 && (
                <div style={{
                  ...
                  color: isOver ? col.color+"88" : "#111e35",
                  fontSize:12,
                  border:`2px dashed ${isOver ? col.color+"44" : "#111e35"}`,
              ...
              {colLeads.map(lead => {
                ...
                return (
                  <div
                    ...
                    style={{
                      background: dragging?.id===lead.id ? "#1a2a44" : "#111a2e",
                  ...
                  >
                    ...
                    <div style={{display:"flex",gap:5,flexWrap:"wrap",marginBottom:6}}>
                      ...
                      {src && (
                        <span style={{
                          fontSize:10,padding:"2px 7px",borderRadius:5,
                          background:"rgba(255,255,255,0.15)",color:"#3a5a80",fontWeight:600,
                        }}>{src.icon}</span>
                      )}
                      {lead.amount && (
...
function LeadsApp() {
  ...
  const TABS = [
    ...
    { id:"pipeline", icon:"⚡", label:"Воронка",    badge: leads.length||null },
  ...
  return (
    <div style={{
      ...
      color:"rgba(255,255,255,0.85)", fontFamily:"'DM Sans','SF Pro Display',-apple-system,sans-serif",
      display:"flex", flexDirection:"column",
    }}>
      <style>{`@import url('https://fonts.googleapis.com/css2?family=DM+Sans:ital,opsz,wght@0,9..40,300;0,9..40,400;0,9..40,500;0,9..40,600;0,9..40,700&display=swap');*{box-sizing:border-box}::-webkit-scrollbar{width:4px}::-webkit-scrollbar-track{background:#0b1120}::-webkit-scrollbar-thumb{background:#1a2a40;border-radius:4px}select option{background:#111a2e}`}</style>

      {/* HEADER */}
      <div style={{
        background:"#0d1628", borderBottom:"1px solid #111e35",
        padding:"14px 18px 0",
      }}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:14}}>
          <div style={{display:"flex",alignItems:"center",gap:10}}>
            ...
            <div>
              ...
              <div style={{fontSize:10,color:"rgba(255,255,255,0.18)",fontWeight:600,letterSpacing:"0.06em"}}>
          ...
          {/* Mini stats + sync */}
          <div style={{display:"flex",gap:8,alignItems:"center"}}>
            {newCount>0&&<div style={{background:"#3b82f618",border:"1px solid #3b82f644",color:"#007AFF",borderRadius:10,padding:"4px 10px",fontSize:11,fontWeight:700}}>🔵 {newCount} новых</div>}
          ...
          {TABS.map(t=>{
            ...
            return (
              <button key={t.id} onClick={()=>setTab(t.id)} style={{
                ...
                color:active?"#007AFF":"rgba(255,255,255,0.28)",
                fontSize:13,fontWeight:active?700:500,
                cursor:"pointer",fontFamily:"inherit",
                ...
                {t.label}
                {t.badge&&<span style={{
                  background:"rgba(255,255,255,0.09)",color:"#007AFF",
      ...
      {/* CONTENT */}
      <div style={{flex:1,display:"flex",flexDirection:"column",minHeight:0,overflowY:tab==="new"?"auto":"hidden",overflowX:tab==="pipeline"?"auto":"hidden"}}>
...
export default function App(){
  ...
  return(
    <AuthCtx.Provider value={auth}>
      ...
      <div style={{height:"100dvh",background:"#0C0C0E",display:"flex",flexDirection:"column",fontFamily:"'DM Sans',-apple-system,sans-serif",overflow:"hidden"}}>
        {/* CONTENT */}
        ...
        <div style={{
          ...
          {[
            {id:"dispatch",icon:"📋",label:"Заказы"},
            {id:"leads",icon:"💼",label:"Лиды"},
          ].map(t=>{
            ...
            return(
              <button key={t.id} onClick={()=>switchTo(t.id)} style={{
                ...
                <span style={{fontSize:22,lineHeight:1,opacity:active?1:0.4}}>{t.icon}</span>
                <span style={{fontSize:10,fontWeight:active?700:400,color:active?"#FFFFFF":"rgba(255,255,255,0.4)",letterSpacing:"0.01em"}}>{t.label}</span>
                {active&&<div style={{width:4,height:4,borderRadius:"50%",background:"#007AFF",marginTop:1}}/>}
              </button>
            );
          })}
        </div>
