...
function mergeDebts(localD, cloudD, tomb) {
  ...
  const m = (a, b) => {
    ...
    (b || []).forEach(x => { if (x && x.id && !tomb["debt_" + x.id] && !map[x.id]) map[x.id] = x; });
    ...
    <SyncCtx.Provider value={{ syncing, lastSync, needsReauth, resume }}>
      {needsReauth && (
        <div onClick={resume} style={{
          ...
          background:"#241a05", borderBottom:"1px solid rgba(245,162,58,0.4)",
          color:"#F5A23A", fontSize:12, fontWeight:700, textAlign:"center",
...
function ExecutorFilter({ value, onChange, executors }) {
  return (
    <div style={{ display:"flex", gap:6, marginBottom:14, flexWrap:"wrap", alignItems:"center" }}>
      <span style={{ fontSize:10, fontWeight:700, color:"rgba(255,255,255,0.22)", letterSpacing:"0.07em", textTransform:"uppercase", marginRight:2 }}>Исполнитель</span>
      {[["all","Все"], ...executors.map(e=>[e,e])].map(([id,label])=>(
        <button key={id} onClick={()=>onChange(id)} style={{
          padding:"5px 13px", borderRadius:20, flexShrink:0,
          border:`1.5px solid ${value===id?"#34D368":"rgba(255,255,255,0.1)"}`,
          background: value===id?"rgba(52,211,104,0.12)":"transparent",
          color: value===id?"#34D368":"rgba(255,255,255,0.45)",
...
// ═══════════════════════════════════════════════════════════
function ServiceStatsView({ orders: allOrders }) {
    ...
    <div style={{flex:1,overflowY:"auto",WebkitOverflowScrolling:"touch",minHeight:0,padding:"18px 20px",paddingBottom:"calc(120px + env(safe-area-inset-bottom))"}}>
      ...
      <div style={{ background: "rgba(52,199,89,0.08)", border: "1px solid rgba(52,211,104,0.2)", borderRadius: 12, padding: "14px 16px", marginBottom: 12 }}>
        ...
        <select value={payExecutor} onChange={e => setPayExecutor(e.target.value)}
          style={{ width:"100%", background:"#181819", border:"1px solid rgba(255,255,255,0.1)",
            ...
            color: payExecutor ? "rgba(255,255,255,0.85)" : "rgba(255,255,255,0.35)" }}>
      ...
      {showManual && (
        <div style={{ background: "rgba(255,59,48,0.08)", border: "1px solid #ef444433", borderRadius: 12, padding: "14px 16px", marginBottom: 14 }}>
          ...
          <select value={manualExecutor} onChange={e => setManualExecutor(e.target.value)}
            ...
            borderRadius:8, fontSize:13, padding:"9px 12px", outline:"none", fontFamily:"inherit",
            boxSizing:"border-box", marginBottom:8, WebkitAppearance:"none", appearance:"none",
              color: manualExecutor ? "rgba(255,255,255,0.85)" : "rgba(255,255,255,0.35)" }}>
            <option value="">Исполнитель (необязательно)</option>
            {executors.map(ex => <option key={ex} value={ex}>{ex}</option>)}
          </select>
          <div style={{ display: "flex", gap: 8 }}>
            ...
            {[...payments].sort((a,b) => (b.date||"").localeCompare(a.date||"")).map(p => (
              <div key={p.id} style={{
                ...
                <div style={{ fontSize: 12 }}>
                  ...
                  {p.executor && <span style={{ color: "#34D368", marginLeft: 8, fontWeight: 600 }}>· {p.executor}</span>}
            ...
            {[...manualDebts].sort((a,b) => (b.month||"").localeCompare(a.month||"")).map(d => (
              <div key={d.id} style={{
                ...
                <div style={{ fontSize: 12 }}>
                  ...
                  {d.executor && <span style={{ color: "#34D368", marginLeft: 8, fontWeight: 600 }}>· {d.executor}</span>}
...
function LeadsApp() {
  ...
  return (
    <div style={{
      height:"100%", minHeight:0, background:"#0A0A0B",
