...
async function calendarCreateEvent(token, order) {
  ...
  const calendarId = encodeURIComponent(localStorage.getItem("calendar_id") || "primary");
...
function CalendarSyncSettings({ auth, showSettings, setShowSettings }) {
  ...
  return (
    <div>
      <div style={{ fontSize:11, fontWeight:700, color:"rgba(255,255,255,0.35)", letterSpacing:"0.07em", textTransform:"uppercase", marginBottom:8 }}>
        Google Calendar — автовыгрузка заказов
      </div>
      {auth?.token ? (
        <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
          {/* Toggle */}
          <div style={{ display:"flex", alignItems:"center", gap:10 }}>
            ...
            <span style={{ fontSize:12, color: enabled ? "rgba(255,255,255,0.75)" : "rgba(255,255,255,0.35)" }}>
              {enabled ? "Включено — каждый заказ → Google Calendar" : "Выключено"}
            </span>
          </div>
          ...
          <div style={{ fontSize:11, color:"rgba(255,255,255,0.22)", lineHeight:1.4 }}>
            Авторизован: {auth.user?.email || "Google аккаунт"}
          </div>

          {enabled && (
            <div style={{ fontSize:11, color:"rgba(52,199,89,0.8)", lineHeight:1.4 }}>
              ✓ При сохранении заказа с датой — событие создаётся автоматически
            </div>
          )}
        </div>
      ) : (
        <div style={{ fontSize:12, color:"rgba(255,255,255,0.35)", lineHeight:1.6 }}>
          Войди через Google (кнопка в правом верхнем углу) — затем включи автовыгрузку здесь.{"\n"}
          <span style={{ color:"#F5A23A", fontWeight:600 }}>
            Нужен Client ID с разрешением calendar.events в Google Cloud Console.
          </span>
        </div>
      )}
    </div>
...
function CalendarIdField() {
  const [calId, setCalId] = useState(() => localStorage.getItem("calendar_id") || "primary");
  const save = (v) => { setCalId(v); localStorage.setItem("calendar_id", v || "primary"); };
  return (
    <div>
      <div style={{ fontSize:11, color:"rgba(255,255,255,0.3)", marginBottom:4 }}>Календарь ID (оставь «primary» для основного)</div>
      <input
        value={calId}
        ...
        style={{
          width:"100%", background:"#181819",
          border:"1px solid rgba(255,255,255,0.1)", borderRadius:8,
          color:"rgba(255,255,255,0.75)", fontSize:12, padding:"7px 10px",
...
function DispatchApp() {
  ...
  const autoSyncToCalendar = async (order) => {
    ...
    try {
      const created = await calendarCreateEvent(token, order);
      if (created?.id) {
    ...
    <div style={{
      ...
      {showSettings && (
        <div style={{
          ...
          <CalendarSyncSettings auth={auth} showSettings={showSettings} setShowSettings={setShowSettings} />
      ...
      {activeView === "home" ? (
        ...
        <div style={{ flex:1, overflowY:"auto", WebkitOverflowScrolling:"touch", padding:"20px", paddingBottom:"calc(100px + env(safe-area-inset-bottom))", display:"flex", flexDirection:"column", gap:20 }}>
          ...
          {/* Google Calendar */}
          <CalendarSyncSettings auth={auth} showSettings={true} setShowSettings={()=>{}} />
  ...
  const saveOrder = (updated, prevOrder) => {
    ...
    if (newNext && newNext !== prevNext) {
      ...
      if (!alreadyExists) {
        ...
        if (token) calendarCreateEvent(token, repeatOrder).catch(() => {});
      }
    }
  };
